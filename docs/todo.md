...
- [ ] **Mapping issues lista prijavljuje pjesme kojima mapiranje radi** — `cloud/app/database.py:1084` (`get_mapping_fail_candidates`), render u `cloud/app/routers/insights.py:64`
  Heuristika broji svaki `played`/`no_scrobble` događaj u prozoru i na 2+ proglasi pjesmu sumnjivom. Ali `played` je vrlo često samo utrka s vremenom: worker provjeri Last.fm minutu nakon starta pjesme, a Last.fm scrobblu prihvati tek nakon pola pjesme (ili 4 minute) i backdatea je na vrijeme početka. U trenutku provjere scrobbla ne postoji, minutu-dvije kasnije postoji — a događaj ostaje zapisan kao "stara scrobbla".
  Potvrđeno na produ 2026-08-04, Nebula Orionis - Regeneration Process (`6wYWmkVQz1azLZrRYFJ9pL`): događaj 2026-07-30 08:42:04 UTC ima `days_ago=181` (tada zadnja scrobbla 29. siječnja), a Last.fm danas ima scrobblu istog dana u 08:41. Mapiranje je cijelo vrijeme radilo. Ovo je glavni izvor lažnih uzbuna na listi — i razlog zašto je nastalo 16 aliasa identičnih Spotify nazivu (klik na Add alias umjesto Dismiss).
  Fix: pri renderiranju liste za svakog kandidata dohvatiti scrobble povijest (`user.gettrackscrobbles`, artist + naziv) i odbaciti događaje koji imaju scrobblu u okolici svog timestampa (nekoliko minuta tolerancije); ako ostane manje od 2 sumnjiva događaja, kandidat ispada. Na `LASTFM_ERROR` NE filtrirati (mrežna greška ne smije sakriti stvarne nalaze). Cijena: 1 Last.fm poziv po kandidatu, lista je kratka.
