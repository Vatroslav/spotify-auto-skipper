...
- [x] **Mapping issues lista prijavljuje pjesme kojima mapiranje radi** — DONE (v3.20.3, PR #78)
  Heuristika je brojala svaki `played`/`no_scrobble` događaj i na 2+ proglasila pjesmu sumnjivom, ali to je najčešće bila utrka s vremenom: worker provjeri Last.fm minutu nakon starta pjesme, a Last.fm scrobblu prihvati tek nakon pola pjesme (ili 4 minute) i backdatea je na vrijeme početka. Novi modul `cloud/app/mapping_fails.py` re-provjerava događaje protiv scrobbli koje Last.fm ima **sada**, u dva prolaza: (1) jedan indeks nedavnih scrobbli (`user.getRecentTracks`) za cijeli prozor — ~24 requesta bez obzira na broj kandidata, matcha doslovni scrobblani naziv; (2) `user.getTrackScrobbles` samo za ono što preživi, jer tamo Last.fm primijeni vlastito razrješavanje naziva (dijakritika). Indeks se održava inkrementalno, uz refetch zadnjih 60 min (backdate bi inače pao kroz watermark). Događaj sa scrobblom u prozoru -30 min / +5 min ispada; kandidat s manje od 2 preostala događaja ispada s liste. Na Last.fm grešci se ne filtrira ništa.
  Izmjereno na produ pri deployu: lista 55 → 23 (prozor 30 dana), 32 ispala, nijedan novi. Ispali su i `Nebula Orionis - Regeneration Process` (prihvatni kriterij) i `Windir - Soknardalr`. Ono što je ostalo su stvarni mismatchevi naziva — `Hallways Of Enchanted Ebony` vs `... - 2016 - Remaster`, `Like Falling Leaves` vs `... - Remastered`, `Grey-Haired Steppe` vs `... - Сивий Степ`. Trajanje: 12.6 s prvi render nakon restarta (gradi indeks), 1.8 s svaki sljedeći; frontend za to vrijeme pokazuje "Checking candidates against Last.fm...". Dismiss i Add alias ne osvježavaju listu (samo maknu redak) — inače bi svaka akcija ponovo pokrenula provjeru za sve preostale kandidate.
...
- [ ] **Android Auto kontroler** — ručne komande (Check Now, Pause, Don't Skip, Liked, Remove) s ekrana auta.
  ...
  - [x] Faza 1 (backend device token auth) — branch `feat/device-token-auth`, PR #77, v3.21.0. Čeka deploy na VPS i test s telefona prije Faze 2.
