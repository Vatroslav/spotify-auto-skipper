...
# SSH deploy to VPS
ssh root@autoskipper.duckdns.org "cd /opt/spotify-auto-skipper/cloud && git pull && docker compose up -d --build"
```
...
Self-hosted web app that auto-skips Spotify tracks based on Last.fm listening history. Deployed with Docker + Caddy on a Hetzner VPS at `autoskipper.duckdns.org`.
