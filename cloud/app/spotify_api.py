...
class SpotifyClient:
    ...
    async def get_current_user_id(self) -> str | None:
        """Fetch the authenticated user's Spotify ID (cached after first call)."""
    ...
    async def get_playlist_tracks(self, playlist_id: str) -> list[dict]:
        ...
        params = {"fields": "items(track(id,name,artists(name),uri)),next", "limit": 100}
        while url:
    ...
    async def create_playlist(self, user_id: str, name: str, description: str = "") -> dict | None:
        """Create a new private playlist. Returns {id, url} or None."""
